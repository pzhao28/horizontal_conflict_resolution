...
def main():
    iter_step = 0
    path = {}
    fig, axs = plt.subplots(num_iteration, 2, figsize = (7,9), sharex=True)
    i = 0
    for ac in init_plan:
    ...
    while iter_step < num_iteration:
        ...
        for ac in order:
            ...
            axs[i, j].plot([0, 0], [y_range[0], y_range[1]] , '--r', label='original flight plan')
            ...
            axs[i, j].set_ylim(y_range[0], y_range[1])
