...
def heuristic_polyline_cos(pos, head, own_name): #only one turn point is considered
    pos = np.array(pos)
    turn_point = init_plan[own_name][1][0]
    end_point = init_plan[own_name][1][1]
    inter_angle = math.pi - (init_plan[own_name][3][0] - init_plan[own_name][3][1])
    a = math.tan(init_plan[own_name][3][1] - inter_angle/2)
    b = turn_point[1] - a*turn_point[0]
    waypoint = turn_point
    if a > 0:
        if pos[1] < a * pos[0] + b:
            waypoint = end_point
...
def AStarSearch(intruder, own_name):
    ...
    if type(init_plan[own_name][3]) == float:
        init_head = init_plan[own_name][3]
        end_pos = init_plan[own_name][1]
    else:
        init_head = init_plan[own_name][3][0]
        end_pos = init_plan[own_name][1][-1]
    ...
    if type(init_plan[own_name][3]) == float:
        ...
        F[start] = heuristic_polyline_cos(start, init_head, own_name)
    ...
    while len(openVertices) > 0:
        ...
        for neighbourVertex in neighbour: #ToDo2: get_vertex_neighbours function
            ...
            if type(init_plan[own_name][3]) == float:
                heuristic_fun = heuristic_cos
            else:
                heuristic_fun = heuristic_polyline_cos
            H = heuristic_fun(currentPos, neighbourHead, own_name)
...
def main():
    ...
    for ac in init_plan:
        if ac == 'ac1':
            path[ac] = np.arange(0, maxTimeStep + 100)
        else:
    ...
    fig, ax = plt.subplots()
    ax.plot(*zip(*path['ac1']), '-k', *zip(*path['ac2']), '-r')
    x1 = [init_plan['ac1'][2][0], init_plan['ac1'][1][0][0], init_plan['ac1'][1][1][0]]
    y1 = [init_plan['ac1'][2][1], init_plan['ac1'][1][0][1], init_plan['ac1'][1][0][1]]
    ax.plot(x1, y1, '--.k')
    x2 = [init_plan['ac2'][2][0], init_plan['ac2'][1][0]]
    y2 = [init_plan['ac2'][2][1], init_plan['ac2'][1][1]]
    ax.plot(x2,y2,'--.r')

    ar1_xy1 = path['ac1'][20]
    ar1_xy2 = path['ac1'][21]
    d_ar1 = np.array(ar1_xy2) - np.array(ar1_xy1)
    ar2_xy1 = path['ac2'][20]
    ar2_xy2 = path['ac2'][21]
    d_ar2 = np.array(ar2_xy2) - np.array(ar2_xy1)
    ar1 = plt.arrow(ar1_xy1[0], ar1_xy1[1], d_ar1[0], d_ar1[1], head_width=20, head_length=15, fc='k', ec='k')
    ar2 = plt.arrow(ar2_xy1[0], ar2_xy1[1], d_ar2[0], d_ar2[1], head_width=20, head_length=15, fc='r', ec='r')
    ax.add_artist(ar1)
    ax.add_artist(ar2)
    circle1 = plt.Circle(ar1_xy2, 25, color='k', fill=False, ls='--')
    circle2 = plt.Circle(ar2_xy2, 25, color='r', fill=False, ls='--')
...
action_cost = [0.1, 0]
init_plan = {'ac1': [[1, 0], [[0, 0], [400,0]], [-500+100*math.sqrt(2), -500+100*math.sqrt(2)], [45 * math.pi / 180, 0]], 'ac2': [[1, -200], [400, 200], [-300, -500], 45 * math.pi / 180]}  # key: name of aircraft, value: [[a,b], waypoint in y=ax+b, [init_pos], init_head]
#init_plan = {'ac1': [[1, 0], [400, 400], [-500+100*math.sqrt(2), -500+100*math.sqrt(2)], 45 * math.pi / 180], 'ac2': [[1, -200], [400, 200], [-300, -500], 45 * math.pi / 180]}
AA = -0.01
CC = 30
DD = 0.1
EE = 0.001
