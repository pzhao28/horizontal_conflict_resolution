...
def main():
    ...
    while iter_step < num_iteration:
        ...
        for ac in order:
            ...
            yticks = np.arange(xstart, xend + 1, 100)
    ...
    for i in np.arange(0,num_ac):
        key = 'ac{}'.format(i+1)
        path_init[key] = path_init[key][0:max_len]
        if key not in order:
            clr = 'r'
        else:
            clr = 'k'
        ax2.plot(*zip(*path_init[key]), '--', color=clr)
        ax2.plot((init_plan[key][1][0], init_plan[key][1][0]),
                 (init_plan[key][1][1], init_plan[key][1][1]), '--.', color=clr)
        data = path[key][0:max_len]
        arrow_x1 = data[0][0]
        arrow_x2 = data[1][0]
        arrow_y1 = data[0][1]
        arrow_y2 = data[1][1]
...
action_cost = [0.1, 0]
init_plan = init_flight_plan(5, 500)
