...
def AStarSearch(intruder, own_name, gamma2):
...
def main():
    ...
    for gamma in [1, 10, 50, 100, 250, 1000]:
        iter_step = 0
        path = {}
        gamma2 = gamma**2
        for ac in init_plan:
        ...
        while iter_step < num_iteration:
            for ac in order:
                ...
                path[ac], act = AStarSearch(intruder_path, ac, gamma2)
                iter_step += 1
        row = i//2
        col = i - row*2
        ...
        axs[row, col].set_title(r'$\gamma=${}'.format(gamma/5))
