...
def main():
    ...
    clr = ['b', 'g', 'r', 'c', 'm', 'y', 'k', 'w']
    i = 0
    for gamma in np.arange(50,60,10):
        gamma2 = gamma**2
        iter_step = 0
        path = {}
        ...
        while iter_step < num_iteration:
            for ac in order:
                ...
                xlabel = 'update for aircraft {}'.format(j+1)
                axs[i, j].set_xlabel(xlabel)
                axs[i, j].set_ylabel(ylabel)'''
            iter_step += 1
        #clr = (random.random(), random.random(), random.random())
        axs.plot(*zip(*path['ac1']),color = clr[i])
        axs.plot(*zip(*path['ac2']),color = clr[i], label=r'$\gamma=${}'.format(gamma))
        i += 1
        axs.legend()
        axs.set_ylim(y_range[0] / 2, y_range[1] / 2)
        axs.set_xlim(x_range[0], x_range[1])
        xstart, xend = axs.get_xlim()
        xticks = np.arange(xstart, xend + 1, 100)
        axs.set_xticks(xticks)
        axs.set_xticklabels(xticks / 5)
        ystart, yend = axs.get_ylim()
        yticks = np.linspace(ystart, yend, 3)
        axs.set_yticks(yticks)
        axs.set_yticklabels(yticks / 5)
        #axs.legend()
...
action_space = np.array([-2 * math.pi / 180, 0, 2 * math.pi / 180])  # heading angle in rad. Nagative sign indicates right hand of the aircraft.
#action_cost = [0.1, 0]
