...
def st_cst(intruder_pos, own_pos, sep_tmp, gamma2_tmp): #for calculation
    own_pos_temp = np.array(own_pos)
    dist2 = np.sum((intruder_pos - own_pos_temp)**2, 1)
    dist = np.sqrt(dist2)
    #print(np.isnan(dist[dist < SEP]).any())
    if len(dist[dist < sep_tmp]) == 0:
        state_cost = np.sum(np.exp(-(dist-SEP)**2 / gamma2_tmp))
    else:
...
def AStarSearch(intruder, own_name, sep_tmp, gamma2_tmp):
    ...
    while len(openVertices) > 0:
        ...
        for neighbourVertex in neighbour: #ToDo2: get_vertex_neighbours function
            ...
            candidateG = st_cst(intruder_pos, neighbourPos, sep_tmp, gamma2_tmp)
...
def main():
    ...
    while iter_step < num_iteration:
        ...
        for ac in order:
            intruder_name = list(init_plan.keys())
            intruder_name.remove(ac)
            intruder_path = {}

            if iter_step == 0:
                if k == 0:
                    intruder_path.update({'ac_virtual': path_virtual[intruder_name[0]]})
                    k += 1
                else:
                    intruder_path.update({intruder_name[0]: path[intruder_name[0]]})
                    intruder_path.update({'ac_virtual': path_virtual[intruder_name[0]]})
                sep_tmp = SEP/2
                gamma2_tmp = gamma2/4
            else:
                ...
                gamma2_tmp = gamma2
                intruder_path.update({intruder_name[0]: path[intruder_name[0]]})
            path[ac], act = AStarSearch(intruder_path, ac, sep_tmp, gamma2_tmp)
