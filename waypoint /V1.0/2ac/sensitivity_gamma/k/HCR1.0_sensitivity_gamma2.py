...
def heuristic_cos(pos, head, own_name, cc):
...
def st_cst(intruder_pos, own_pos, gamma2): #for calculation
...
def AStarSearch(intruder, own_name, gamma2, cc):
    ...
    G[start] = [0, init_head]
    F[start] = heuristic_cos(start, init_head, own_name, cc) # ToDo 1: heuristic function
    ...
    while len(openVertices) > 0:
        ...
        for neighbourVertex in neighbour: #ToDo2: get_vertex_neighbours function
            ...
            candidateG = st_cst(intruder_pos, neighbourPos, gamma2)
            ...
            H = heuristic_cos(currentPos, neighbourHead, own_name, cc)
    ...
    for cc in [10, 20, 30, 40, 50, 60]:
        ...
        while iter_step < num_iteration:
            for ac in order:
                ...
                path[ac], act = AStarSearch(intruder_path, ac, gamma2, cc)
                iter_step += 1
        row = i//2
        col = i - row*2
        axs[row, col].plot(*zip(*path['ac1']), color='k', label='aircraft 1')
        axs[row, col].plot(*zip(*path['ac2']), color='r', label='aircraft 2')
        arrow_pos_x1 = [*zip(*path['ac1'])][0][-10]
        arrow_pos_y1 = [*zip(*path['ac1'])][1][-10]
        arrow_pos_x2 = [*zip(*path['ac2'])][0][-10]
        arrow_pos_y2 = [*zip(*path['ac2'])][1][-10]
        axs[row, col].plot(arrow_pos_x1, arrow_pos_y1, '->k', arrow_pos_x2, arrow_pos_y2, '-<r')
        ...
        axs[row, col].add_artist(circle1)
        axs[row, col].add_artist(circle2)

        axs[row, col].set_ylim(y_range[0] / 2.5, y_range[1] / 2.5)
        axs[row, col].set_xlim(x_range[0], x_range[1])
        xstart, xend = axs[row, col].get_xlim()
        xticks = np.arange(xstart, xend + 1, 100)
        axs[row, col].set_xticks(xticks)
        axs[row, col].set_xticklabels(xticks / 5)
        ystart, yend = axs[row, col].get_ylim()
        yticks = np.linspace(ystart, yend, 3)
        axs[row, col].set_yticks(yticks)
        axs[row, col].set_yticklabels(yticks / 5)
        axs[row, col].set_title(r'$k=${}'.format(cc))
        axs[row, col].legend(loc=2, prop={'size': 5})
        axs[row, col].set_xlabel('nautical mile')
        axs[row, col].set_ylabel('nautical mile')
        i += 1
    #plt.axis('equal')
...
#action_cost = [0.1, 0]
init_plan = {'ac1': [[0, 0], [x_range[1], 0], [x_range[0], 0], 0 * math.pi / 180], 'ac2': [[0, 0], [x_range[0], 0], [x_range[1], 0], 180 * math.pi / 180]}
