...
def main():
    ...
    while iter_step < num_iteration:
        ...
        axs[i].plot(*zip(*path['ac1']), '-k', label='path planning of aircraft1')
        axs[i].plot(*zip(*path['ac2']), '-r', label='path planning of aircraft2')
        axs[i].plot([x_range[0], x_range[1]],[0, 0],'--k', label='original flight plan')
        axs[i].legend(loc=2, prop={'size': 5})
        ...
        #plt.show()
        #ax = plt.gca()
        axs[i].plot(arrow_pos_x1, arrow_pos_y1, '->k',  arrow_pos_x2, arrow_pos_y2, '-^r')
        ...
        axs[i].set_ylim(y_range[0]/2, y_range[1]/2)
        axs[i].set_xlim(x_range[0], x_range[1])
        xstart, xend = axs[i].get_xlim()
        xticks = np.arange(xstart, xend+1, 100)
        axs[i].set_xticks(xticks)
        axs[i].set_xticklabels(xticks/5)
        ystart, yend = axs[i].get_ylim()
        yticks = np.linspace(ystart, yend, 3)
        axs[i].set_yticks(yticks)
        axs[i].set_yticklabels(yticks/5)
