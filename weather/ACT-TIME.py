...
def st_cst(intruder_pos, weather_pos, weather_radius, own_pos): #for calculation
    own_pos_temp = np.array(own_pos)
    dist2 = np.sum((intruder_pos - own_pos_temp)**2, 1)
    dist = np.sqrt(dist2)
    distance_weather = np.sqrt(np.sum((weather_pos - own_pos)**2))
    weather_cost = 0
    if len(distance_weather[distance_weather<weather_radius]) == 0:
        weather_cost = 0
    else:
        weather_cost = sys.maxsize
    ...
    return state_cost + weather_cost
...
def AStarSearch(intruder, own_name):
    ...
    while len(openVertices) > 0:
        ...
        for neighbourVertex in neighbour: #ToDo2: get_vertex_neighbours function
            ...
            candidateG = st_cst(intruder_pos,weather_pos, weather_radius, neighbourPos)
    ...
    while iter_step < num_iteration:
        for ac in order:
            ...
            for i in np.arange(0, num_ac):
                plt_path, = ax.plot(*zip(*path['ac{}'.format(i)]), '-k')
                plt_plan, = ax.plot((init_plan['ac{}'.format(i)][1][0], init_plan['ac{}'.format(i)][2][0]),
                        (init_plan['ac{}'.format(i)][1][1], init_plan['ac{}'.format(i)][2][1]), '--.k')
                ...
                weather = plt.Circle(weather_pos, weather_radius, color='gray', fill=True, ls='-', label='sever weather area')
                ax.add_artist(ar)
                ax.add_artist(weather)

            '''for key in path_init:
                ax.plot(*zip(*path_init[key]), '--k')'''
            plt.ylim(y_range[0], y_range[1])
            plt.xlim(x_range[0], x_range[1])
            #plt.axis('equal')
            ...
            ax.axis('equal')
            #ax.legend('sever weather area')
            plt_path.set_label('path planning')
            plt_plan.set_label('original flight plan')
            #weather.set_label('sever weather area')

            plt.legend()
            plt.legend(handles=[plt_path, plt_plan, weather], labels=['path planning', 'original flight plan', 'severe weather area'])
