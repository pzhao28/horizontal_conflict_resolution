...
def main():
    ...
    path = {}
    intruder_path = {}
    cost = np.zeros((num_ac, num_iteration))
    for ac in init_plan:
    ...
    while iter_step < num_iteration:
        ...
        for ac in order:
            ...
            if iter_step == 0:
                if ac == order[0]:
                    intruder_path.update({ac: path[ac]})
                    continue
            ...
            path[ac], act, cost[jj][iter_step] = AStarSearch(intruder_path, ac)
            jj += 1
            intruder_path.update({ac: path[ac]})
    ...
    total_cost = cost.sum(0)
    plt.plot(total_cost)

    print(cost)
    '''(x, y) = np.meshgrid(np.arange(cost.shape[0]), np.arange(cost.shape[1]))
    fig3 = plt.figure()
    ax3 = fig3.add_subplot(111, projection='3d')
    ax3.plot_surface(x, y, np.transpose(cost), cmap=plt.cm.coolwarm)
    ax3.set_zlim3d(min_cost, max_cost)'''
    plt.show()
...
action_cost = [0.1, 0]
init_plan = init_flight_plan(6, 500)
