...
def main():
    ...
    while iter_step < num_iteration:
        ...
        for ac in order:
            ...
            fig, ax = plt.subplots()
            for i in np.arange(0, num_ac):
                ax.plot(*zip(*path['ac{}'.format(i+1)]), '--k')
                ax.plot((init_plan['ac{}'.format(i+1)][1][0], init_plan['ac{}'.format(i+1)][1][0]),(init_plan['ac{}'.format(i+1)][1][1], init_plan['ac{}'.format(i+1)][1][1]), '--.k')
...
time_step = 1  # seconds
g_speed = 10.0  # 1 indicates 0.01 nautical mile per second
...
action_cost = [0.1, 0]
init_plan = init_flight_plan(12, 500)
