...
def st_cst(intruder_pos, own_pos): #for calculation
    ...
    dist = np.sqrt(dist2)
    dist_virtrl = np.sqrt(np.sum((own_pos_temp)**2))
    #print(np.isnan(dist[dist < SEP]).any())
    if len(dist[dist < SEP]) == 0:
        state_cost = np.sum(np.exp(-(dist-SEP)**2 / gamma2))
    else:
        state_cost = sys.maxsize
    if len(dist_virtrl[dist_virtrl < SEP]) == 0:
        state_cost += np.sum(np.exp(-(dist_virtrl-SEP)**2 / gamma2))
    else:
        state_cost += sys.maxsize
    return state_cost
...
time_step = 1  # seconds
g_speed = 120  # 1 indicates 0.01 nautical mile per second
...
wayp_radial = 2000
ac_num = 10
init_plan = init_flight_plan(ac_num, wayp_radial)
