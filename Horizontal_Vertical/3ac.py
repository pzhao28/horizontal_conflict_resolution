    ...
    while iter_step < num_iteration:
        for ac in order:
            ...
            for i in np.arange(0, num_ac):
                if i==0:
                    ax.plot(*zip(*path_init['ac{}'.format(i)][:15][:]),'-k')
                    ax.plot(*zip(*path_init['ac{}'.format(i)][15:][:]), '--r')
                    data = path_init['ac{}'.format(i)]
                elif i == 2:
                    ax.plot(*zip(*path['ac{}'.format(i)][:15][:]), '-k')
                    ax.plot(*zip(*path['ac{}'.format(i)][15:][:]), '--g')
                ...
                rect1 = patches.Rectangle((-520, -50), 450, 100, linewidth=0.8, edgecolor='g', facecolor='none')
                ax.add_patch(rect1)
                rect2 = patches.Rectangle((-250, -100), 100, 600, linewidth=0.8, edgecolor='r', facecolor='none')
                ax.add_patch(rect2)
                arrow_x1 = data[17][0]
                arrow_x2 = data[18][0]
                arrow_y1 = data[17][1]
                ...
                ar = plt.arrow(arrow_x1, arrow_y1, arrow_dx, arrow_dy, head_width=20, head_length=15, fc=clr[i], ec=clr[i], label='aircraft {}'.format(label[i]))
        ...
        legend_elements = [Line2D([0], [0], marker='>', color='r', label='aircraft A',
                                  markerfacecolor='r', markersize=7),
                           Line2D([0], [0], marker='>', color='g', label='aircraft B',
                                  markerfacecolor='g', markersize=7),
                           Line2D([0], [0], marker='>', color='b', label='aircraft C',
...
def generate_flight_plan():
    ...
    ac['ac0'] = [[0, 0], [-200, 500], [-200, -400], 90 * math.pi / 180]
    ac['ac1'] = [[0, 0], [0, 500], [0, -200], 90 * math.pi / 180]
    ac['ac2'] = [[0, 0], [-500, 0], [200, 0], 180 * math.pi / 180]
