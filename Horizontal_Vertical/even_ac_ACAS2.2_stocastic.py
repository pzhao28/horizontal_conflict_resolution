...
def main():
    ...
    while step < num_iteration:
        ...
        for key in order:
            ...
            all_ac.update(ac_virtual)
            if step == 0:
                cst_map = world.cost_map(key, order, all_ac, T_to_hit, alpha/2)
            else:
                del all_ac['ac_virtual']
                cst_map = world.cost_map(key, order, all_ac, T_to_hit, alpha)
            #else:
                #cst_map = world.cost_map(key, order, all_ac, T_to_hit, alpha)
            ...
            '''plt_cst_map(cst_map)
            plt.savefig('{}_iteration_cost_map_{}'.format(step, key))
            plt.close()
            #plt.show()
            plt_trvl_plan(trvl_plan)
            plt.savefig('{}_iteration_travel_plan_{}'.format(step, key))
            plt.close()'''
            if step == num_iteration - 1:
                #plt_cst_map(axs[i, 0], cst_map)
                '''im = axs[i, 0].imshow(np.transpose(cst_map), origin='lower', extent=[0, 200, -2000, 2000], aspect="auto")
                #axs[i, 0].set_yticks(np.arange(-1000, 1000, 100))
                #plt.colorbar(subfig, cax = axs[i,0])
                axs[i, 0].set_title('Cost Map of Aircraft A' if i==0 else 'Cost Map of Aircraft B')
                axs[i,0].set_xlabel('Time(s)')
                axs[i,0].set_ylabel('Relative height(ft)')'''
                #plt_trvl_plan(axs[i, 1], trvl_plan)
                axs[i].plot((trvl_plan - start_state[0]) * height_range[2]*50/6, color='r' if i==0 else 'g')
                axs[i].set_title('Path Planning of Aircraft A' if i==0 else 'Path Planning of Aircraft B')
                axs[i].set_ylabel('Relative height(ft)')
...
class Grid:
    ...
    def cost_map(self, own_name, order, all_ac, T_to_hit, alpha):
        cst_data = np.array(self.data, dtype = np.float64)
        for i in range(len(self.x)):
            for j in range(len(self.y)):
                cst_data[i][j] = state_cost(own_name, order, all_ac, T_to_hit, i, self.y[j], alpha)
        return cst_data
...
def state_cost(own_name, order, all_ac, T_to_hit, step, own_height, alpha):
    intruder = dict()
    intruders_plan = []
    for key in all_ac:
        if key == own_name:
    ...
    st_cost = np.sum(np.exp(-(relative_height*(50/6)/alpha)**2) * np.exp(-(time_to_hit/60)**2))
    return st_cost

'''def init_sort():
    init_cost_dic = dict()
    for key in all_ac:
        init_cost = state_cost(key, all_ac, T_to_hit, start_state[2], start_height)
        init_cost_dic[key] = init_cost
    #order = sorted(init_cost_dic)
    order = sorted(init_cost_dic, key=lambda x: init_cost_dic[x], reverse=True)
    return order'''
...
T_to_hit = {'ac1':[0,100,100],'ac2':[100,0,100], 'ac_virtual':[100,100,0]}
...
ac_virtual = ac.set_ac('ac_virtual', 0, 0, init_plan)
all_ac = {**ac1, **ac2}
...
order = ['ac1', 'ac2'] #fixed order! Do not change exept adding or deleting.
num_ac = len(order)
