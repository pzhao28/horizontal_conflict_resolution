...
def AStarSearch(intruder, own_name):
    ...
    while len(openVertices) > 0:
        ...
        g_speed = [init_speed - 1, init_speed, init_speed + 1]
        ...
        for neighbourVertex in neighbour: #ToDo2: get_vertex_neighbours function
            ...
            speed_change = (neighbour[neighbourVertex][3] - init_speed) / time_step
            accelarationCost = ((acc)**2 * BB + (speed_change)**2*10000 )
...
def main():
    ...
    plt.plot(*zip(*path['ac1']), '-k.', *zip(*path['ac2']), '-r.', *zip(*path['ac3']), '-b.',[x_range[0], x_range[1]],[y_range[0], y_range[1]],'--y', [x_range[0], x_range[1]])
...
init_plan = init_flight_plan(3, 500)
AA = -0.01
BB = 1000
CC = 30
world = Grid(x_range, y_range)
order = init_plan.keys()
num_iteration = 10
maxTimeStep = 70
SEP = 25
gamma2 = 2500
