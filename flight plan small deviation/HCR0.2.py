...
def st_cst(intruder_pos, own_pos): #for calculation
    ...
    state_cost = np.sum(np.exp(-dist2/800))
    return state_cost
...
def main():
    ...
    '''dist_map = world.dist([199,0])
    neighbour = world.get_vertex_neighbours([0,0],0,action_space,0)
    ac1 = Aircraft("ac1", 0, 0, 0, 0)
    dic_ac1 = ac1.set_ac()

    H = heuristic([10,10],'ac1')'''
    #path, act = AStarSearch(np.zeros(200), 'ac1')
    ac2_init_plan = np.array(list(zip(np.arange(x_range[1], -500, -10), np.zeros(100))))
    path, act = AStarSearch({'ac2': ac2_init_plan}, 'ac1')
    #print(act)
    x = np.arange(0, 510, 10)
    y = np.zeros(51)
    plt.plot(*zip(*path), '-k.', x, y, '-r.')
    plt.ylim(-250, 250)
    plt.show()
    path2, act2 = AStarSearch({'ac1':path}, 'ac2')
    plt.plot(*zip(*path2), '-r.', *zip(*path), '-k.')
    plt.ylim(-250, 250)
    plt.show()

    path3, act3 = AStarSearch({'ac2': path2}, 'ac1')
    plt.plot(*zip(*path3), '-k.', *zip(*path2), '-r.')
    plt.ylim(-250, 250)
    plt.show()
    path4, act4 = AStarSearch({'ac1': path3}, 'ac2')
    plt.plot(*zip(*path4), '-r.', *zip(*path3), '-k.')
    plt.ylim(-250, 250)
    plt.show()
    path5, act5 = AStarSearch({'ac2': path4}, 'ac1')
    plt.plot(*zip(*path5), '-k.', *zip(*path4), '-r.')
    plt.ylim(-250, 250)
    plt.show()
    path6, act6 = AStarSearch({'ac1': path5}, 'ac2')
    plt.plot(*zip(*path6), '-r.', *zip(*path5), '-k.')
    plt.ylim(-250, 250)
    plt.show()

    path7, act7 = AStarSearch({'ac2': path6}, 'ac1')
    plt.plot(*zip(*path7), '-k.', *zip(*path6), '-r.')
    plt.ylim(-250, 250)
    plt.show()
    path8, act8 = AStarSearch({'ac1': path7}, 'ac2')
    plt.plot(*zip(*path8), '-r.', *zip(*path7), '-k.')
    plt.ylim(-250, 250)
    plt.show()
    print(path)
    #path2, act2 = AStarSearch(act, 'ac2')
    #edge = world.edge()


x_range = np.array([0, 500, 1])  # 1 indicates 0.01 nautical mile
...
AA, BB, CC, DD = 0.005, 0.1, 0.008, 0.001
world = Grid(x_range, y_range)
